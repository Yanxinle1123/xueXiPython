...
class CClass:
    ...
    def _gen_c_constructors(self):
        ...
        if len(self._class.constructors) > 0:
            init_params_create = ', '.join(
                [init_params_create] + [p.name for p in self._class.constructors[0].parameters])
    ...
    def _gen_c_methods(self):
        ...
        for m in self._class.methods.values():
            params = ', '.join(
                [self._class.name + ' *self'] + [_parse_type(p.type) + ' ' + p.name for p in m.parameters])
